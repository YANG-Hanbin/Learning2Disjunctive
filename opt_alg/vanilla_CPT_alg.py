...
class CuttingPlaneTreeAlgorithm(CuttingPlaneMethod):
    def __init__(self, instanceName, maxIteration=100, OutputFlag=0, Threads=1, MIPGap=0.0, TimeLimit=3600, MIPFocus=2,
                 ...
                 addCutToMIP=False, number_branch_var=2, normalization='SNC', additional_param=None):
    ...
    def branching_variable_selection(self):
        ...
        if number_of_noninteger > 0:
            maxKey = max(self.non_integer_vars[self.iteration - 1], key=self.non_integer_vars[
                self.iteration - 1].get)  # find the integer variables that have the largest distance to the nearest integer
            maxDistance = self.non_integer_vars[self.iteration - 1][maxKey]

        if number_of_nonbinary > 0:
            tmp_maxKey = max(self.non_binary_vars[self.iteration - 1], key=self.non_binary_vars[
                self.iteration - 1].get)  # find the integer variables that have the largest distance to the nearest integer
