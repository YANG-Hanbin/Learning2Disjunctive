...
class CuttingPlaneMethod:
    ...
    def cut_generation(self):
        ...
        cglp.update()
        # Set objective
        cglp.setObjective(gp.quicksum(pi[v] * self.lp_relaxation.getVarByName(v).x for v in self.varName) - pi0,
                          GRB.MINIMIZE)
    ...
    def print_iteration_info(self, cut_time=0.0, iteration_time=0.0, overall=0.0):
        ...
        if self.iteration == 1:
            ...
            print(
                f'---------------------------------------------------------------------------------------------------------------------------------')
            print(
                f'|  Iter  |  # fractional var  |  current value  |  Relative Improvement  |  Overall Improvement  |  Iter Time  |  Overall Time  |')
            print(
                f'---------------------------------------------------------------------------------------------------------------------------------')
        else:
            print('| ' + '{:7d}'.format(self.iteration - 1) + '| ' + '{:19d}'.format(
                len(self.non_integer_vars[self.iteration - 1]) + len(
                    self.non_binary_vars[self.iteration - 1])) + '| ' + '{:16.4f}'.format(
                self.lp_obj_value[self.iteration - 1]) + '| ' + '{:22.4f}'.format(
                abs(self.lp_obj_value[self.iteration - 1] - self.lp_obj_value[self.iteration - 2]) / self.lp_obj_value[
                    self.iteration - 1] * 100) + ' | ' + '{:21.4f}'.format(
                abs(self.lp_obj_value[self.iteration - 1] - self.lp_obj_value[0]) / self.lp_obj_value[
                    0] * 100) + ' | ' + '{:11.4f}'.format(iteration_time) + ' | ' + '{:14.4f}'.format(overall) + ' |')
